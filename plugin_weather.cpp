...
PluginWeather::PluginWeather():PluginInterface("weather", "MÈtÈo",BunnyZtampPlugin)
{
...
void PluginWeather_Worker::run()
{
	...
	else
	{
		...
		{
			QByteArray where = TTSManager::CreateNewSound(QString("MÈtÈo pour %1.").arg(sCity), "claire");
			message += "MU "+where+"\nPL 3\nMW\n";
			if(current!=0)
			{
				QByteArray maintenant = TTSManager::CreateNewSound("actuellement, ", "claire");
				QByteArray temperature = TTSManager::CreateNewSound(QString::number(iCurrentTemp) + " degrÈs", "claire");
			...
			if(forecast!=0)
			{
				...
				QByteArray temperature = TTSManager::CreateNewSound("entre "+QString::number(iForecastLow) + " et "+QString::number(iForecastHigh) + " degrÈs", "claire");
